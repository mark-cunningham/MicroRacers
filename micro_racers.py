...
car_image = pygame.image.load("red_car.png").convert_alpha()
computer_car_image = pygame.image.load("blue_car.png").convert_alpha()
...
def main():
    ...
    level = 1
    full_track = get_track(level)



    start_grid_x = full_track[0][0][0] * TRACKBLOCKSIZE
    start_grid_y = full_track[0][0][1] * TRACKBLOCKSIZE + TRACK_HEIGHT
    ...
    lights_y = lights_loc[1] * TRACKBLOCKSIZE + TRACK_HEIGHT
    ...
    while True: # main game loop
        ...
        for track_piece in range(len(full_track)):
            ...
            track_piece_top = full_track[track_piece][0][1] * TRACKBLOCKSIZE - TRACKBLOCKSIZE/2 + TRACK_HEIGHT
            track_piece_right = full_track[track_piece][0][0] * TRACKBLOCKSIZE - TRACKBLOCKSIZE / 2 + TRACKBLOCKSIZE
            track_piece_bottom = full_track[track_piece][0][1] * TRACKBLOCKSIZE - TRACKBLOCKSIZE / 2 + TRACKBLOCKSIZE + TRACK_HEIGHT
        ...
        for track_piece in range(len(full_track)):
            ...
            track_y = full_track[track_piece][0][1] * TRACKBLOCKSIZE - TRACKBLOCKSIZE/2 + TRACK_HEIGHT
        ...
        display_scoreboard(laps, computer_laps, level)

        if race_won is True:
            display_end_race_message(laps, computer_laps, level)
...
def get_track(level):
    ...
    if level == 1:
        track = [[[3, 1], start_grid_image], [[4, 1], horiz_straight_image], [[5, 1], horiz_straight_image],
        ...
        track = [[[2, 1], start_grid_image], [[3, 1], right_down_image], [[3, 2], down_right_image],
...
def display_scoreboard(laps, computer_laps, level):

    # Display scores and level
    scoreboard_back_rect = (0, 0, SCREENWIDTH, SCOREBOARDHEIGHT)
    pygame.draw.rect(game_screen, BLACK, scoreboard_back_rect)

    scoreboard_red_car_rect = (0, 0, SCOREBOARD_CAR_WIDTH, SCOREBOARDHEIGHT)
    pygame.draw.rect(game_screen, RED, scoreboard_red_car_rect)

    scoreboard_blue_car_rect = (SCREENWIDTH - SCOREBOARD_CAR_WIDTH, 0, SCOREBOARD_CAR_WIDTH, SCOREBOARDHEIGHT)
...
def display_end_race_message(laps, computer_laps, level):
    ...
    race_won_background_rect = (
    (SCREENWIDTH - RESULT_BLOCK_WIDTH) / 2, (SCREENHEIGHT - RESULT_BLOCK_HEIGHT) / 2, RESULT_BLOCK_WIDTH, RESULT_BLOCK_HEIGHT)
    pygame.draw.rect(game_screen, BLACK, race_won_background_rect)

    game_screen.blit(text,
                     [(SCREENWIDTH - text_rect.width) / 2, (SCREENHEIGHT - RESULT_BLOCK_HEIGHT) / 2 + TEXTLINEHEIGHT * 5])

    text = font.render('RESULT', True, (WHITE))
    text_rect = text.get_rect()
    game_screen.blit(text, [(SCREENWIDTH - text_rect.width) / 2, (SCREENHEIGHT - RESULT_BLOCK_HEIGHT) / 2 + TEXTLINEHEIGHT * 2])

    text = font.render(winner_text, True, (WHITE))
    text_rect = text.get_rect()
    game_screen.blit(text, [(SCREENWIDTH - text_rect.width) / 2, (SCREENHEIGHT - RESULT_BLOCK_HEIGHT) / 2 + TEXTLINEHEIGHT * 3])
